  ...
  useEffect(() => {
    socket.on("newMessage", (message) => {
      if (selectedChat?._id === message.chatId) {
        ...
        setUsers((prev) =>
            prev.map((u) =>
                u._id === message.sender
                    ? { ...u, latestMessage: { text: message.text, sender: message.sender } }
                    : u
            )
        );
      }
      ...
      async function fetchAllUsers() {
        try {
          const { data } = await axios.get(
            `/api/user/all?search=${searchQuery}`,
          );
          setUsers(data.users);
        } catch (error) {
          console.log(error);
        ...
        <div className="w-1/3 p-4 border-r border-border h-full overflow-y-auto">
          ...
          <div className="w-full mt-4">
            <ul>
              {users?.map((item) => (
                <li
                  className="flex items-center gap-3 text-text p-3 hover:bg-border/20 cursor-pointer"
                  key={item._id}
                  ...
                  <div className="flex flex-col">
                    <div className="flex items-center gap-2">
                      ...
                      {onlineUsers.includes(item._id) && (
                        <span className="w-2 h-2 ml-1 rounded-full bg-green-400 shrink-0"></span>
                      )}
                    </div>
                    {item.latestMessage?.text && (
                      <span className="text-sm text-muted">
                        {item.latestMessage.sender === user._id ? `You: ${item.latestMessage.text}` : `${item.latestMessage.text}`}
        ...
        <div className="w-2/3 h-full flex flex-col">
          {selectedChat ? (
            <>
              ...
              <div ref={messageContainerRef} className="flex-1 overflow-y-auto p-4 flex flex-col gap-2">
                {messages.map((msg) => (
                  <div
                    key={msg._id}
                  ...
                  >
                    ...
                    <span
                      className={`text-[0.64rem] tracking-[0.08em] text-muted whitespace-nowrap`}
                    >
